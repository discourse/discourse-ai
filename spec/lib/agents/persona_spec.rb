...
RSpec.describe DiscourseAi::Agents::Agent do
  ...
  it "can parse string that are wrapped in quotes" do
    ...
    tool_instance =
      DiscourseAi::Agents::Artist.new.find_tool(tool_call, bot_user: nil, llm: nil, context: nil)
  ...
  it "can correctly parse arrays in tools" do
    ...
    tool_instance =
      DiscourseAi::Agents::DallE3.new.find_tool(tool_call, bot_user: nil, llm: nil, context: nil)
    expect(tool_instance.parameters[:prompts]).to eq(["cat oil painting", "big car"])
  end
  ...
  describe "available agents" do
    it "includes all agents by default" do
      ...
      expect(DiscourseAi::Agents::Agent.all(user: user).map(&:superclass)).to contain_exactly(
        DiscourseAi::Agents::General,
        DiscourseAi::Agents::Artist,
      ...
      expect(DiscourseAi::Agents::Agent.all(user: admin).map(&:superclass)).to contain_exactly(
        DiscourseAi::Agents::General,
        DiscourseAi::Agents::Artist,
        DiscourseAi::Agents::Creative,
        DiscourseAi::Agents::DiscourseHelper,
      ...
      AiAgent.find(
        DiscourseAi::Agents::Agent.system_agents[DiscourseAi::Agents::General],
  ...
  describe "#craft_prompt" do
    ...
    context "when RAG is running with a question consolidator" do
      ...
      it "will run the question consolidator" do
        ...
        custom_agent =
          DiscourseAi::Agents::Agent.find_by(id: custom_ai_agent.id, user: user).new
    ...
    context "when a agent has RAG uploads" do
      ...
      context "when the agent has examples" do
        ...
        it "includes them before the context messages" do
          custom_agent =
            DiscourseAi::Agents::Agent.find_by(id: examples_agent.id, user: user).new
