  ...
  module AiBot
    class Playground
      ...
      def self.schedule_reply(post)
        ...
        if post.topic.private_message?
          mentionables =
            AiAgent.allowed_modalities(user: post.user, allow_agentl_messages: true)
        else
        ...
        if bot_user
          ...
          if agent_id
            agent = DiscourseAi::Agents::Agent.find_by(user: post.user, id: agent_id.to_i)
          end
      ...
      def schedule_bot_reply(post)
        agent_id =
          DiscourseAi::Agents::Agent.system_agents[bot.agent.class] || bot.agent.class.id
        ::Jobs.enqueue(
          :create_ai_reply,
          post_id: post.id,
